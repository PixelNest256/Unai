...
function renderTurn(turnData, stream = false) {
  ...
  const { userBar, botBar } = buildActionBars(turn_id, branch_index, branch_count, userBubble, botBubble, metaEl);
  // Insert userBar right after userMsg (before botMsg)
  chatEl.insertBefore(userBar, botMsg);
  chatEl.appendChild(botBar);

  // Fill bot content
  if (stream) { typeText(botBubble, branch.bot.content); }
  else { botBubble.textContent = branch.bot.content; }
...
function renderTurn(turnData, stream = false) {
  ...
  // ── action bar (shared for the turn, appended after bot) ──
  const actionBar = buildActionBar(turn_id, branch_index, branch_count, userBubble, botBubble, metaEl);
  chatEl.appendChild(actionBar);
...
/* ── Build action bars (user + bot, separate) ── */

// Returns { userBar, botBar } — caller inserts them in the right place
function buildActionBars(turn_id, branchIndex, branchCount, userBubble, botBubble, metaEl) {

  // ── User action bar (below user bubble) ──────────────────────────────────
  const userBar = document.createElement('div');
  userBar.className = 'turn-actions user-actions';
  userBar.dataset.turnId = turn_id;
  ...
  const editBtn = makeActionBtn(
    ...
    () => startEdit(turn_id, userBubble, botBubble, metaEl, userBar, botBar)
  );
  userBar.appendChild(editBtn);

  const copyUserBtn = makeActionBtn(
    `<svg width="12" height="12" viewBox="0 0 12 12" fill="none">
  ...
  userBar.appendChild(copyUserBtn);

  // ── Bot action bar (below bot bubble) ────────────────────────────────────
  const botBar = document.createElement('div');
  botBar.className = 'turn-actions bot-actions';
  botBar.dataset.turnId = turn_id;

  const nav = document.createElement('div');
  nav.className = 'branch-nav';
  nav.dataset.turnId = turn_id;
  updateBranchNav(nav, branchIndex, branchCount, turn_id, userBubble, botBubble, metaEl, botBar);
  botBar.appendChild(nav);

  const copyBotBtn = makeActionBtn(
    `<svg width="12" height="12" viewBox="0 0 12 12" fill="none">
      <rect x="4" y="4" width="7" height="7" rx="1.5" stroke="currentColor" stroke-width="1.2"/>
      <path d="M1 8V2a1 1 0 0 1 1-1h6" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/>
    </svg>Copy`,
  ...
  const regenBtn = makeActionBtn(
    ...
    () => regenerateTurn(turn_id, botBar, userBubble, botBubble, metaEl)
  );
  botBar.appendChild(regenBtn);

  // ── Hover wiring ─────────────────────────────────────────────────────────
  // User side: user bubble + user bar
  let uh = 0;
  const showU = () => { uh++; userBar.classList.add('visible'); };
  const hideU = () => { uh = Math.max(0, uh - 1); if (uh === 0) userBar.classList.remove('visible'); };
  [userBubble.closest('.msg') || userBubble, userBar].forEach(el => {
    el.addEventListener('mouseenter', showU);
    el.addEventListener('mouseleave', hideU);
  });

  // Bot side: bot bubble + bot bar
  let bh = 0;
  const showB = () => { bh++; botBar.classList.add('visible'); };
  const hideB = () => { bh = Math.max(0, bh - 1); if (bh === 0) botBar.classList.remove('visible'); };
  [botBubble.closest('.msg') || botBubble, botBar].forEach(el => {
    el.addEventListener('mouseenter', showB);
    el.addEventListener('mouseleave', hideB);
  });

  return { userBar, botBar };
}
...
async function regenerateTurn(turn_id, bar, userBubble, botBubble, metaEl) {
  ...
  try {
    ...
    const data = await res.json();
    botBubble.innerHTML = '';
    const nav = botBar.querySelector('.branch-nav');
    updateBranchNav(nav, data.branch_index, data.branch_count, turn_id, userBubble, botBubble, metaEl);
...
/* ── Edit user message ────────────────────────── */
function startEdit(turn_id, userBubble, botBubble, metaEl, userBar, botBar) {
  if (isWaiting) return;
  ...
  sendEditBtn.addEventListener('click', () => submitEdit(turn_id, textarea, userBubble, botBubble, metaEl, userBar, botBar, row));

  textarea.addEventListener('keydown', e => {
    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); submitEdit(turn_id, textarea, userBubble, botBubble, metaEl, bar, row); }
...
async function submitEdit(turn_id, textarea, userBubble, botBubble, metaEl, userBar, botBar, row) {
...
async function send() {
  ...
  try {
    ...
    typingEl.remove();
    appendMsgFallback('bot', 'Connection error. Make sure the server is running.');
  }
